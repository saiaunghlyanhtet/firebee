...
/* BPF Test Framework - Inspired by Cilium's test infrastructure */
...
/* Convert __LINE__ to string */
#define STRINGIZE(x) STRINGIZE2(x)
#define STRINGIZE2(x) #x
#define LINE_STRING STRINGIZE(__LINE__)

/* Use an array map with 1 key and a large value size as buffer for results */
struct {
...
/* Protobuf wire type markers for encoding test results */
...
/* Helper macros for logging arguments (up to 6 arguments) */
#define __bpf_log_arg0(ptr, arg) do {} while (0)
#define __bpf_log_arg1(ptr, arg) \
        *(ptr++) = MKR_LOG_ARG; *(__u64 *)(ptr) = arg; ptr += sizeof(__u64)
#define __bpf_log_arg2(ptr, arg, args...) \
        *(ptr++) = MKR_LOG_ARG; *(__u64 *)(ptr) = arg; \
...
({                                                                             \
        ...
        *(suite_result_cursor++) = sizeof(____fileline) - 1 + sizeof(____fmt); \
        __builtin_memcpy(suite_result_cursor, ____fileline, sizeof(____fileline) - 1); \
        suite_result_cursor += sizeof(____fileline) - 1;                       \
        __builtin_memcpy(suite_result_cursor, ____fmt, sizeof(____fmt));       \
...
/* Mark the current test as failed and exit */
...
/* Log a message and fail the test */
...
#define test_init()                                                       \
        ...
        {                                                                 \
                __u32 __key = 0;                                          \
                suite_result_cursor = bpf_map_lookup_elem(&test_result_map, &__key); \
...
do {                                                                       \
        ...
        *(suite_result_cursor++) = sizeof(____name);                       \
        __builtin_memcpy(suite_result_cursor, ____name, sizeof(____name)); \
        suite_result_cursor += sizeof(____name);                           \
                                                                           \
...
/* Finish test suite and return result */
#define test_finish()           \
        } while (0);            \
        return suite_result

/* Define test program section */
#define CHECK(name) SEC("xdp")
